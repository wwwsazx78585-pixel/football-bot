...
async def match_detail(call: types.CallbackQuery):
    ...
    data = get_matches_with_stats(league)
    date, match, odds, stats = data["matches"][match_idx]
...
async def stats(call: types.CallbackQuery):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="⭐ Бавария", callback_data="team_bayern")],
...
async def team_stats(call: types.CallbackQuery):
    team = call.data.replace("team_", "")
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⬅️ Статистика", callback_data="stats")],
        [InlineKeyboardButton(text="🏠 Главное", callback_data="start")]
    ])
    await call.message.edit_text(
        f"📈 <b>{team.upper()}</b>\n\n"
        f"✅ Форма 10 матчей: 8-1-1\n"
        f"⚽ Голы: 2.8/1.2\n"
        f"⛳ Угловые: 7.2/4.1\n"
        f"🟨 Карточки: 1.8/2.2\n"
        f"📊 ТБ2.5: <b>80%</b>\n\n"
...
async def main():
    print("🚀 v8.2-FIX — ПРО-ФУТБОЛ Бот запущен!")
